...
void print_jsmn_test_functions(const char *file)
{
  jsmnint_t rv, i;
  size_t len;
  char *json;
  ...
  printf("// %s\n", file);
  printf("static void %s(void **state)\n{\n%*s(void)state; // unused\n", file, 4, "");
  printf("%*sconst char *js = \"", 4, "");
  print_escaped_string(json, len);
  printf("\";\n");
  printf("%*sassert_int_equal(jsmn_parse(&p, js, strlen(js), t, %d), %d);\n", 4, "", rv, rv);
  printf("%*stokeq(js, t, %d,\n", 4, "", rv);
  for (i = 0; i < rv; i++) {
    token = &tokens[i];
...
void print_jsmn_test_functions_fail(const char *file)
{
  jsmnint_t rv, i;
  size_t len;
  char *json;
  ...
  print_escaped_string(json, len);
  printf("\";\n");
  printf("%*sassert_int_equal(jsmn_parse(&p, js, strlen(js), NULL, 0), (jsmnint_t)%d);\n", 4, "", rv);
  printf("}\n\n");
...
//  "n_structure_open_array_object.json",
  ...
  for (i = 0, len = sizeof(test_parser_i) / 80; i < len; i++) {
    print_jsmn_test_functions(test_parser_i[i]);
  }

  for (i = 0, len = sizeof(test_parser_n) / 80; i < len; i++) {
    print_jsmn_test_functions_fail(test_parser_n[i]);
  }

  for (i = 0, len = sizeof(test_parser_y) / 80; i < len; i++) {
    print_jsmn_test_functions(test_parser_y[i]);
  }
}
