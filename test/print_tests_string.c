...
char *file_contents(const char *file, size_t *len)
{
  FILE *fp;
  size_t result;
  char *buffer;
  ...
  fp = fopen (file, "r" );
  if (fp == NULL) { fprintf(stderr, "File error: %s", file); exit(EXIT_FAILURE); }
  fseek(fp, 0, SEEK_END);
  *len = ftell(fp);
  rewind(fp);
  buffer = (char*)malloc(sizeof(char) * *len + 1);
  if (buffer == NULL) { fprintf(stderr, "Memory error"); exit(EXIT_FAILURE); }
  result = fread(buffer, 1, *len, fp);
  if (result != *len) { fprintf(stderr, "Reading error"); exit(EXIT_FAILURE); }
  buffer[*len] = '\0';
  fclose (fp);
...
void print_escaped_string(const char *str, const size_t len)
{
  char c;
...
void print_tests_string(const char *file)
{
  jsmnint_t rv, i;
  size_t len;
  char *json;

  char filename[15 + strlen(file)];
  sprintf(filename, "JSONTestSuite/%s", file);
  json = file_contents(filename, &len);
  ...
  printf("  /* %s */\n", file);
  printf("  check(parse(\"");
  print_escaped_string(json, len);
  printf("\", %d, %d,", rv, rv);
  for (i = 0; i < rv; i++) {
    token = &tokens[i];
    if (token->type & JSMN_OBJECT) {
      printf("\n%*sJSMN_OBJECT, %d, %d, %d", 14, "", token->start, token->end, token->children);
    } else if (token->type & JSMN_ARRAY) {
      printf("\n%*sJSMN_ARRAY,  %d, %d, %d", 14, "", token->start, token->end, token->children);
...
void print_tests_string_fail(const char *file)
{
  jsmnint_t rv, i;
  size_t len;
  char *json;
  ...
  printf("  check(query(\"");
  print_escaped_string(json, len);
  printf("\", %s));\n\n", (rv == JSMN_ERROR_INVAL ? "JSMN_ERROR_INVAL" : "JSMN_ERROR_PART"));
...
//  "n_structure_open_array_object.json",
  ...
  printf("int test_jsmn_test_suite_i_(void) {\n");
  for (i = 0, len = sizeof(test_parser_i) / 80; i < len; i++) {
    print_tests_string(test_parser_i[i]);
  }
  printf("  return 0;\n}\n\n");

  printf("int test_jsmn_test_suite_n_(void) {\n");
  for (i = 0, len = sizeof(test_parser_n) / 80; i < len; i++) {
    print_tests_string_fail(test_parser_n[i]);
  }
  printf("  return 0;\n}\n\n");

  printf("int test_jsmn_test_suite_y_(void) {\n");
  for (i = 0, len = sizeof(test_parser_y) / 80; i < len; i++) {
    print_tests_string(test_parser_y[i]);
  }
  printf("  return 0;\n}\n\n");
}
